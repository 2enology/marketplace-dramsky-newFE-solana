...
export const getEscrowBalance = async (connection: Connection) => {
  ...
  const escrowATA = await getAssociatedTokenAccount(escrowVault, USDT_TOKEN_MINT);
  console.log("Escrow:", escrowVault.toBase58(), "EscrowATA:", escrowATA.toBase58());
  const token = (await getTokenAccountBalance(escrowATA, connection)) as number;

  return {
    sol: res,
    token: token * USDT_TOKEN_DECIMAL,
  };
};
